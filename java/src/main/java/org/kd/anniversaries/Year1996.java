...
public class Year1996 extends Year {
    ...
    public void draw(long frame, Scene scene) {
        ...
        if (frame < this.startingFrame + 0.4 * Year.DEFAULT_DURATION) {
            ...
            font2.draw(screen.batch2, "Garii Kasparov beat IBM Deep Blue 4 times.", 90, 535);

        } else if (frame < this.startingFrame + 1.2 * DEFAULT_DURATION) {
            font2.draw(screen.batch2, "Sun Microsystems released JAVA in January 1996", 90, 535);

            if (frame > this.startingFrame + 0.45 * DEFAULT_DURATION) {
                font2.draw(screen.batch2, "Java is like a wheel", 100, 255);
                font2.draw(screen.batch2, "invented long time ago,", 100, 235);
                font2.draw(screen.batch2, "but still needed.", 100, 215);
            }
            if (frame > this.startingFrame + 0.6 * DEFAULT_DURATION) {
                font2.draw(screen.batch2, "This demo is actually", 100, 165);
                font2.draw(screen.batch2, "written in Java. LOL", 100, 145);
            }
            screen.backgroundTexture = this.java;

        } else if (frame < this.endFrame) {
            if (frame < this.startingFrame + 1.35 * DEFAULT_DURATION)
                font2.draw(screen.batch2, "In December 1996 flagship product of Sony", 90, 535);
            else font2.draw(screen.batch2, " became available in South Africa :D", 90, 535);
